 ...
 */
function testSortingMethod(sortMethod, debug) {
	const testCases = createTestCases(100);
	return testCases.reduce((success, arr, index) => {
		console.log(`### Test case ${index + 1} ###`)
		if (debug) console.log('Input:', arr)
		const res = sortMethod(arr)
...
function test(sortMethod, debug = false) {
	if (testSortingMethod(sortMethod, debug)) {
